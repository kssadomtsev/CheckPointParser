...
def create_host_plain(host_plain, cur):
    ...
    rows = cur.fetchone()
    host_plain_obj = network_objects.host_plain(rows[0], rows[1], rows[2].lower(), rows[3])
...
def create_network(network, cur):
    ...
    rows = cur.fetchone()
    network_obj = network_objects.network(rows[0], rows[1], rows[2].lower(), rows[3])
...
def create_network_object_group(network_object_group, cur):
    ...
    for s_ in get_members(network_object_group, cur):
        ...
        logging.info(row_str_)
        print(s_ + " " + row_str_)
        if row_str_ == "host_plain" and s_ not in list_network_objects:
            list_network_objects.append(s_)
            logging.info("Creating host_plain " + s_)
            create_host_plain(s_, cur)
        elif row_str_ == "network" and s_ not in list_network_objects:
            list_network_objects.append(s_)
            logging.info("Creating network " + s_)
        ...
        elif row_str_ == "network_object_group" and s_ not in list_network_objects:
            list_network_objects.append(s_)
            logging.info("Creating network_object_group " + s_)
    ...
    rows = cur.fetchone()
    print(rows)
    network_object_group_obj = network_objects.network_object_group(rows[0], rows[1],
                                                                    rows[2].lower(), rows[3])
...
def create_network_objects():
    ...
    if conn is not None:
        cur = conn.cursor()
        cur.execute("SELECT DISTINCT src FROM global_policy")
        rows1 = cur.fetchall()
        cur.execute("SELECT DISTINCT dst FROM global_policy")
        rows2 = cur.fetchall()
        rows = rows1 + rows2
        s = set()
        list = []
        ...
        output.print_to_xlsx(groups_list, "groups_r80_global.xlsx")
